 ...
 * </ul>
 */
class SemanticVersionImpl implements Version {
	...
	public SemanticVersionImpl(String version, boolean storeX) throws IllegalArgumentException {
		...
		if (version.endsWith(".")) {
			throw new IllegalArgumentException("Negative version number component found!");
		...
		if (componentStrings.length < 1) {
			throw new IllegalArgumentException("Did not provide version numbers!");
		}
		...
		for (int i = 0; i < componentStrings.length; i++) {
			...
			if (storeX) {
				if (compStr.equals("x") || compStr.equals("X") || compStr.equals("*")) {
					if (prerelease != null) {
						throw new IllegalArgumentException("Pre-release versions are not allowed to use X-ranges!");
					}
					...
					throw new IllegalArgumentException("Interjacent wildcard (1.x.2) are disallowed!");
				}
			}

			if (compStr.trim().isEmpty()) {
				throw new IllegalArgumentException("Missing version number component!");
			}
			...
			try {
				...
				if (components[i] < 0) {
					throw new IllegalArgumentException("Negative version number component '" + compStr + "'!");
				}
			} catch (NumberFormatException e) {
				throw new IllegalArgumentException("Could not parse version number component '" + compStr + "'!", e);
			}
		}

		if (storeX && components.length == 1 && components[0] == COMPONENT_WILDCARD) {
			throw new IllegalArgumentException("Versions of form 'x' or 'X' not allowed!");
		}
	...
	public int compareTo(@NotNull Version other) {
		if (!(other instanceof SemanticVersionImpl)) {
			return getFriendlyString().compareTo(other.getFriendlyString());
		}

		SemanticVersionImpl o = (SemanticVersionImpl) other;
